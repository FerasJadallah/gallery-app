...
export default function SignupPage() {
  ...
  const fieldHasError = useCallback(
    (field: keyof SignupValues) => Boolean(errors[field]),
    [errors]
  );
  ...
  const onSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      if (!parsed.success) {
        ...
        showAlert(
          "error",
          "Please fix the highlighted issues and try signing up again."
        );
        return;
      }
      ...
      <div className="w-full max-w-md space-y-6 rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="space-y-2 text-center">
          ...
          <p className="text-sm text-slate-600">
            Enter your details to start exploring the gallery.
          </p>
        </div>
